...
public class TabContatoDAO extends BuscaBandoDados {
    ...
    public List<TabContatoVO> getContatoVOList() {
        ...
        if (contatoVOList != null)
            for (TabContatoVO contato : contatoVOList)
                addObjetosPesquisa(contato);
    ...
    void addObjetosPesquisa(TabContatoVO contato) {
        ...
        contato.setTelefoneVOList(telefoneVOList);

        List<TabEmailHomePageVO> emailHomePageVOList = new ArrayList<>();
        for (String strCodEmailHomePage : contato.getEmailHomePage_ids().split(";"))
